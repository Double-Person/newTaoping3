<template>
	<view class='sign'>
			...
			<view class="imgbg background-image" style="background-image: url(/static/jifen/j2.png);" >
				<view class="laba" >
					<view class="laba1">
						...
						<span> 您的{{userInfo.integral || 0}}积分即将过期 </span>
					</view>
				</view>
				<view class="yuan">
					<p>我的积分</p>
				...
				<view class="tuoyuan">
					<p class='but on' v-if="userInfo.isDaySign">已签到</p>
					<form @click="goSign" report-submit='true' v-else>
						<p class='but' formType="submit">立即签到</p>
					</form>
				</view>
				<view class="jfdh">
		...
		<!-- pst积分规则-弹窗部分开始 -->
		<!-- <view class="tank" v-show="isshow">
				<scroll-view class="tank2" scroll-y="true" @scroll = "monScroll">
					<text class="jfgztext">积分规则</text>
					<text class="jfgztext2">
						一、积分获取方式
						1.新用户
						新用户可免费获赠一定的随机体验积分。新用户是指没有在本商城提交过订单的用户，同一账号、同一手机号、同一终端设备号或其它合理显示为同一用户的情形，均视为同一用户。
						2.签到
						用户首日签到可领取50积分，连续签到每日可领取100积分，若签到中断则重新计算。
						3.购买返积分商品
						购买返积分商品的用户，选择在线支付成功购买后可立返积分。
						4.关注公众号
						关注公众号可以获得一定的奖励积分。
						5.分享送好礼活动
						...
						3. 如您对商品的标题、价格、详情等信息内容有疑问，请在购买前咨询商城在线客服，客服工作时间为周一至周日早9:00至晚21:00。
						...
						13. 在法律允许范围内，本商城拥有最终解释权。如有任何疑问，请在第一时间联系商城在线客服或拨打400-098-3678咨询热线，在线客服工作时间为：周一至周日早9:00至晚21:00，感谢您对本商城的支持与信赖，祝您购物愉快！
					</text>
				</scroll-view>
				<view class="queren" @click="ShowHidden">
					我知道了
				</view>
		</view> -->
		<!-- pst积分规则-弹窗部分结束 -->
...
<script>
	...
	export default {
		...
		methods: {
			// 弹窗部分开始-pst
			// monScroll(e){
			// 	var tempScroll =  e.detail.scrollTop
			// 	if(tempScroll >= 40){
			// 		this.isTop = true
			// 	} else {
			// 		this.isTop = false
...
<style scoped>
	...
	.imgbg{width: 100%;height: 260px;z-index: 10;position: relative;background-image: linear-gradient(140deg,#16a9ce,#1bdeba);}
	.background-image {
