...
<template>
	<view>
		<view class="gimgbg gbackground-image sign" style="background-image: url(/static/jifenhuangou/g1.png);">
			<view class="jfhgrl">
				<view class="jfhg2">
					<span class='item' v-for="(item,index) in signSystemList" :key="index">
						<p :class='(index+1) == signSystemList.length ? "rewardTxt" : ""'>{{item.title}}</p>
						<!-- <p class='venus' :class="(index + 1 === signSystemList.length ? 'reward' : '') + ' ' +(sign_index >= index + 1 ? 'venusSelect' : '')"></p> -->
						<p class='num' :class='item.is_sgin ? "on" : ""'>+{{item.integral}}</p>
					</span>
					<!-- <span>
						<p>10月</p>
						<p>26日</p>
			...
			<view class="dqjf">
				<view>
					<span>
						<image class="mckf" src="/static/mycenter/m7.png" mode=""></image>
						<span>
							<p>{{userInfo.integral || 0}}</p>
							<p class="nddqjf">您的当前积分</p>
						</span>
					</span>
					<span class="mrjf"  @click="goSign">每日签到领积分</span>
				</view>
				<view class="xsjfhg">
					<p>限时积分兑换</p>
			...
			<view class="cxjfkf">
				<span>
					<image @click="seekdd" src="/static/jifenhuangou/g2.png" mode=""></image>
				</span>
				<span>
					<image @click="showBase" src="/static/jifenhuangou/g3.png" mode=""></image>
				</span>
				<span>
					<image src="/static/jifenhuangou/g4.png" mode=""></image>
				</span>
			</view>
		</view>
		...
		<!-- //pst--选项卡结束 -->
		<view>
			<tabBar2 :pagePath="pages"></tabBar2>
		</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				pages:"/pages/jifen/jifen",
				startTime:'2020-10-26 05:02:22',
				enTime:'2020-10-26 12:00:00',
...
<style scoped>
	.gimgbg{width: 100%;height: 140px;z-index: 10;position: relative;background-image: linear-gradient(140deg,#16a9ce,#1bdeba);}
	.gbackground-image {
	...
	.jfhg2 span{background-color: #212e46;}
	.jfhg2 span p{margin-bottom: 0.3rem;padding: 0.2rem 0.4rem;}
	
	.rewardTxt{background-color:#5163ff ;color: #fff;}
	...
	.dqjf{
		...
		height: 9rem;
		color: #fff;
		/* background-image: linear-gradient(180deg,#4c77ff,#545bff); */
		background-color: #5163ff;
	}
	.dqjf view:first-child{
		width: 100%;
		display: flex;
		height: 55px;
		line-height: 20px;
	...
	.dqjf view span:first-child{margin: 0 0.5rem;font-size: 0.8rem;display: flex;justify-content: space-around;}
	.dqjf view span:last-child{padding-left: 0.3rem;}
	.nddqjf{font-size: 0.5rem;}
	.mckf{width:2.8rem;height: 2.8rem;}	
	.mrjf{background-color: #00c6c1;height: 2rem;line-height: 2rem;border-radius: 5px;padding: 0.2rem 0.5rem;}
	...
	.xsjfhg p{width: 40%;position: absolute;top: -0.6rem;left: 30%;background-color: #5163ff;text-align: center;}
	...
	.cxjfkf span{width: 32%;}
	.cxjfkf span image{width: 100%;height: 300%;}
	...
	.rxbd11 image:nth-child(2){
		margin-right: 5px;
	}
	.rxbd11 image:nth-child(4){
		margin-right: 10px;
	}
	.rxbd11 view{
		color: #FF3600;
...
<style scoped>
	/deep/ .countdown .time{margin-top: -0.5rem;font-size: 1.5rem;background-color:transparent;}
	/deep/ .countdown{margin-left: 0rem; margin-top: 0.3rem;}
	/deep/ .image-container{margin: 0 auto;}
	/deep/ .countdown .text{color: #fff;}
</style>